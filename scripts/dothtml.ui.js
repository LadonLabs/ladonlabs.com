//Tabs
dot.createWidget("uitabs", function(tabsArray){
	var first = true;
...
dot.createWidget("uitab", function(name, fragmentIdentifier, content){
...
dot.createWidget("uiwrappedlist", function(array, wrapperBuilder){
	return dot.each(array, function(element){
		return wrapperBuilder(element);
	});
});

dot.createWidget("uicenter", function(content){
	//return dot.div(dot.div(content)).style("width: 100%; text-align: center;");
	return dot.div(content).style("width: 100%; text-align: center;");
});

dot.createWidget("uimiddle", function(content){
	return dot.table(dot.tr(dot.td(content).style("vertical-align: middle;"))).style("height: 100%;width: 100%;")
});

//Collapsing margins.
dot.createWidget("uicontent", function(content){
	return dot.uicenter(dot.div(content).style("max-width: 1110px; margin: auto;"));
});
